...
def predict(fname_specialists='net-specialists.pickle', append_name = "base"):
    ...
    for flip in flips:
        for rot in rotations:
            ...
            for model in specialists.values():
                ...
                print "   Multi Log loss = ",multiclass_log_loss(y,y_pred)
                print ""
                ii = ii + 1
                test_aug.delete()
            

    print "Number of models predicted = ",len(predictions_stacked)
    print ""
    ...
    print "FINAL COMBINED Multi Log loss for base = ",multiclass_log_loss(y,predictions_stacked[0])
    print "FINAL COMBINED Multi Log loss for base rotations = ",multiclass_log_loss(y,avg_rot_base)
    print "FINAL COMBINED Multi Log loss for base rotations + Flip = ",multiclass_log_loss(y,avg_rot_flip)
    print "FINAL COMBINED Multi Log loss for all rotations + Flip = ",multiclass_log_loss(y,avg_full_rot_flip)
    print ""

    df = DataFrame(predictions_stacked[0])
    print df.head()
    print df.shape
    df.to_csv("prediction_out_" + append_name + ".csv")

    df_rot = DataFrame(avg_rot_base)
    print df_rot.head()
    print df_rot.shape
    df_rot.to_csv("prediction_out_blended_test_sets_base_rot_" + append_name + ".csv")

    df_rot_flip = DataFrame(avg_rot_flip)
    print df_rot_flip.head()
    print df_rot_flip.shape
    df_rot_flip.to_csv("prediction_out_blended_test_sets_base_rot_flip_" + append_name + ".csv")

    df_full_rot_flip = DataFrame(avg_full_rot_flip)
    print df_full_rot_flip.head()
    print df_full_rot_flip.shape
    df_full_rot_flip.to_csv("prediction_out_blended_test_sets_full_rot_flip_" + append_name + ".csv")
